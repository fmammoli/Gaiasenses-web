...
export default function ClientMap({
  ...
  const prevMarker = useRef<{ latitude?: number; longitude?: number } | null>(
    null
  );
  ...
  }, [marker, pathname, router, searchParams, showPopup, shuffled]);
    ...
    <div className={`h-svh relative isolate bg-black`} id={"total-container"}>
      <Map
        ...
        onMove={(e) => {
          ...
          setMarker({
            latitude: parseInt(center.lat.toString()),
            longitude: parseInt(center.lng.toString()),
          });
          if (showPopup) {
            setShowPopup(false);
          }
