...
import { Image, Renderer, Vector } from "p5";
...
function sketch(p5: P5CanvasInstance<SketchProps & ChaosTreeSketchProps>) {
  ...
  class Particle {
    ...
    constructor(x: number, y: number, size: number, img: Image) {
  ...
  const overlap = (x1: number, y1: number, diam1: number, x2: number, y2: number, diam2: number) => {
    return (
  ...
  const initialize = () => {
    ...
    p5.loadImage(nestedBiome[randomImg], (image) => {
      img = image;
